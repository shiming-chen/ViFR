...
wandb.init(project='ViFR', config='wandb_config/config_sun.yml')
opt = wandb.config
...
dataloader = SUNDataLoader('./', opt.device)
...
optimizerFR = optim.Adam(
    netFR.parameters(), lr=opt.dec_lr, betas=(opt.beta1, 0.999))
optimizer_center = optim.Adam(
    center_criterion.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
...
for i in range(0, opt.pre_iters):
    ...
    out_package = net_PFR(batch_feature)
...
for epoch in range(0,opt.nepoch):
    ...
    if opt.final_classifier == 'softmax':
        zsl = classifier_zero.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses),
                                         ...
                                         netFR=netFR, dec_size=opt.attSize, dec_hidden_size=(opt.latensize*2),
                                         opt=opt)
        acc = zsl.acc
        if best_zsl_acc <= acc:
